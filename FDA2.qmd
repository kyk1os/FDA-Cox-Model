...
duplicates <- raw %>% 
  group_by(athlete_id, year, event, performance) %>% 
  filter(n() > 1) %>% 
  nrow()
...
to_event_family <- function(ev) {
  ...
  case_when(
    ...
    # Separate steeplechase BEFORE hurdles check
    str_detect(ev, "steeple") ~ "Long", 
    
    # Hurdles (pure technical + speed events)
    str_detect(ev, "hurdle") ~ "Hurdles",
    ...
    # Relays - classify by distance
    str_detect(ev, "relay") & str_detect(ev, "4x[18]00|4x15") ~ "Middle",
    str_detect(ev, "relay") ~ "Sprint",
...
# EXCLUDE Unknown events (cross country, non-standard events)
cat("\n=== EXCLUDING UNKNOWN EVENTS ===\n")
unknown_count <- sum(df$event_family == "Unknown")
cat("Dropping", unknown_count, "observations classified as Unknown\n")
cat("Percent excluded:", round(unknown_count/nrow(df)*100, 2), "%\n")

df <- df %>% filter(event_family != "Unknown")

# Summary of event families
cat("\n=== EVENT FAMILY DISTRIBUTION (After Exclusion) ===\n")
family_summary <- df %>% 
